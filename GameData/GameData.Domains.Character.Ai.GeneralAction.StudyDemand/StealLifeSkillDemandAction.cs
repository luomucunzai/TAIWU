...
public class StealLifeSkillDemandAction : IGeneralAction
{
	...
	public void ApplyInitialChangesForTaiwu(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase <= 3)
		{
			monthlyNotificationCollection.AddStealLifeSkillFailure(id, location, id2, skillBookItem.LifeSkillTemplateId);
			ApplyChanges(context, selfChar, targetChar);
			return;
		}
		monthlyNotificationCollection.AddStealLifeSkillSuccess(id, location, id2, skillBookItem.LifeSkillTemplateId);
		if (Phase == 4)
		{
			monthlyEventCollection.AddStealLifeSkillButBeCaught(id, location, id2, 10, BookTemplateId, PageId + 1);
		}
		else
		{
			monthlyEventCollection.AddStealLifeSkillAndEscape(id, location, id2, 10, BookTemplateId, PageId + 1);
		}
	...
	public void ApplyChanges(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase >= 4)
		{
			...
			int dataOffset = secretInformationCollection.AddStealLifeSkill(id, id2, lifeSkillTemplateId);
			int num = DomainManager.Information.AddSecretInformationMetaData(context, dataOffset);
		}
		switch (Phase)
		{
		case 0:
			lifeRecordCollection.AddStealLifeSkillFail1(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			break;
		case 1:
			lifeRecordCollection.AddStealLifeSkillFail2(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			break;
		case 2:
			lifeRecordCollection.AddStealLifeSkillFail3(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			break;
		case 3:
			lifeRecordCollection.AddStealLifeSkillFail4(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			break;
		case 4:
			if (id == taiwuCharId)
			{
			...
			lifeRecordCollection.AddStealLifeSkillSucceed(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			if (id2 != taiwuCharId)
			{
		...
		default:
			...
			selfChar.LearnNewLifeSkill(context, lifeSkillTemplateId, (byte)(1 << (int)PageId));
			lifeRecordCollection.AddStealLifeSkillSucceedAndEscaped(id, currDate, id2, location, 10, BookTemplateId, PageId + 1);
			break;
		}
	}
}
