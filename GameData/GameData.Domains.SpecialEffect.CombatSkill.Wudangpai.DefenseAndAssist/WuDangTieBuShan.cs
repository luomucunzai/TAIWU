...
public class WuDangTieBuShan : DefenseSkillBase
{
	...
	public WuDangTieBuShan(CombatSkillKey skillKey)
		: base(skillKey, 4500)
	{
	}
	...
	private void OnCompareDataCalcFinished(CombatContext context, DamageCompareData compareData)
	{
		if (base.CombatChar == context.Defender && base.CanAffect)
		{
			if (base.IsDirect)
			{
				int num = compareData.OuterDefendValue + compareData.InnerDefendValue;
				compareData.OuterDefendValue = (compareData.InnerDefendValue = num / 2);
			}
			else
			{
				int num2 = compareData.OuterAttackValue + compareData.InnerAttackValue;
				compareData.OuterAttackValue = (compareData.InnerAttackValue = num2 / 2);
			}
