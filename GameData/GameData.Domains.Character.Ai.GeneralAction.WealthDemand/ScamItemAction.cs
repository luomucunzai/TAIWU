...
public class ScamItemAction : IGeneralAction
{
	...
	public void ApplyInitialChangesForTaiwu(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase <= 2)
		{
			monthlyNotificationCollection.AddCheatItemFailure(id, location, id2, TargetItem.ItemType, TargetItem.TemplateId);
		...
		{
			monthlyEventCollection.AddScamLegendaryBook(id, location, id2, (ulong)TargetItem, Phase);
		}
		else
		{
			monthlyEventCollection.AddScamItem(id, location, id2, (ulong)TargetItem);
		}
	...
	public void ApplyChanges(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase >= 4)
		{
			...
			int dataOffset = secretInformationCollection.AddScamItem(id, id2, (ulong)TargetItem);
			int num = DomainManager.Information.AddSecretInformationMetaData(context, dataOffset);
		}
		switch (Phase)
		{
		case 0:
			lifeRecordCollection.AddScamItemFail1(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			break;
		case 1:
			lifeRecordCollection.AddScamItemFail2(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			break;
		case 2:
			lifeRecordCollection.AddScamItemFail3(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			break;
		case 3:
			lifeRecordCollection.AddScamItemFail4(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			break;
		case 4:
		{
			lifeRecordCollection.AddScamItemSucceed(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			if (id2 == taiwuCharId)
			{
				break;
			}
			...
			{
				lifeRecordCollection.AddScamItemFailAndBeatenUp(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
		...
		default:
		{
			...
			DomainManager.Character.TransferInventoryItem(context, targetChar, selfChar, TargetItem, Amount);
			targetChar.ChangeHappiness(context, DomainManager.Item.GetBaseItem(TargetItem).GetHappinessChange());
			lifeRecordCollection.AddScamItemSucceedAndEscaped(id, currDate, id2, location, TargetItem.ItemType, TargetItem.TemplateId);
			break;
		}
		}
	}
