...
public class RobResourceAction : IGeneralAction
{
	...
	public void ApplyInitialChangesForTaiwu(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase <= 2)
		{
			monthlyNotificationCollection.AddRobResourceFailure(id, location, id2, ResourceType);
			ApplyChanges(context, selfChar, targetChar);
		}
		else
		{
			monthlyEventCollection.AddRobResource(id, location, id2, ResourceType, Amount);
	...
	public void ApplyChanges(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (Phase >= 4)
		{
			...
			int dataOffset = secretInformationCollection.AddRobResource(id, id2, ResourceType);
			int num = DomainManager.Information.AddSecretInformationMetaData(context, dataOffset);
		}
		switch (Phase)
		{
		case 0:
			lifeRecordCollection.AddRobResourceFail1(id, currDate, id2, location, ResourceType);
			break;
		case 1:
			lifeRecordCollection.AddRobResourceFail2(id, currDate, id2, location, ResourceType);
			break;
		case 2:
			lifeRecordCollection.AddRobResourceFail3(id, currDate, id2, location, ResourceType);
			break;
		case 3:
			lifeRecordCollection.AddRobResourceFail4(id, currDate, id2, location, ResourceType);
			break;
		case 4:
			lifeRecordCollection.AddRobResourceSucceed(id, currDate, id2, location, ResourceType, Amount);
			if (id2 != DomainManager.Taiwu.GetTaiwuCharId())
			{
				...
				{
					lifeRecordCollection.AddRobResourceFailAndBeatenUp(id, currDate, id2, location, ResourceType, Amount);
		...
		default:
		{
			...
			lifeRecordCollection.AddRobResourceSucceedAndEscaped(id, currDate, id2, location, ResourceType, Amount);
			break;
		}
		}
	}
