...
public class AddDamageByHitType : AssistSkillBase
{
	...
	public unsafe override int GetModifyValue(AffectedDataKey dataKey, int currModifyValue)
	{
		...
		HitOrAvoidInts hitValues2 = character.GetHitValues();
		HitOrAvoidInts avoidValues2 = character.GetAvoidValues();
		if (((hitValues.Items[HitType] > hitValues2.Items[HitType] || avoidValues.Items[HitType] > avoidValues2.Items[HitType]) && dataKey.FieldId == 69) || ((hitValues.Items[HitType] < hitValues2.Items[HitType] || avoidValues.Items[HitType] < avoidValues2.Items[HitType]) && dataKey.FieldId == 102))
		{
			_affected = true;
			return 50;
		}
