...
public class RequestRestoreQiAction : IGeneralAction
{
	...
	{
		return selfChar.GetDisorderOfQi() > 0 && targetChar.GetInventory().Items.ContainsKey(ItemUsed) && selfChar.GetEatingItems().GetAvailableEatingSlotsCount(selfChar.GetCurrMaxEatingSlotsCount()) > 0;
	}
	...
	public void ApplyInitialChangesForTaiwu(DataContext context, Character selfChar, Character targetChar)
	{
		...
		monthlyEventCollection.AddRequestHealDisorderOfQi(id, location, targetChar.GetId(), (ulong)ItemUsed);
	...
	public void ApplyChanges(DataContext context, Character selfChar, Character targetChar)
	{
		...
		if (AgreeToRequest)
		{
			...
			{
				throw new Exception($"Invalid item type {ItemUsed} to restore disorder of Qi for {selfChar.GetId()}");
			}
			...
			lifeRecordCollection.AddRequestHealDisorderOfQiSucceed(id, currDate, id2, location, ItemUsed.ItemType, ItemUsed.TemplateId);
			SecretInformationCollection secretInformationCollection = DomainManager.Information.GetSecretInformationCollection();
			int dataOffset = secretInformationCollection.AddAcceptRequestRestoreDisorderOfQi(id2, id);
		...
		else
		{
			...
			lifeRecordCollection.AddRequestHealDisorderOfQiFail(id, currDate, id2, location, ItemUsed.ItemType, ItemUsed.TemplateId);
			SecretInformationCollection secretInformationCollection2 = DomainManager.Information.GetSecretInformationCollection();
			int dataOffset2 = secretInformationCollection2.AddRefuseRequestRestoreDisorderOfQi(id2, id);
