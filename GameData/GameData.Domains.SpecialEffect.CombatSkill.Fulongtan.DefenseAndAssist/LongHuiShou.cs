...
public class LongHuiShou : DefenseSkillBase
{
	...
	public LongHuiShou(CombatSkillKey skillKey)
		: base(skillKey, 14501)
	{
	}
	...
	private void OnBounceInjury(DataContext context, int attackerId, int defenderId, bool isAlly, sbyte bodyPart, sbyte outerMarkCount, sbyte innerMarkCount)
	{
		if (attackerId == base.CharacterId && base.CanAffect)
		{
			if (base.IsDirect)
			{
				base.CombatChar.ChangeNeiliAllocation(context, 0, 1);
			}
			else
			{
				base.CurrEnemyChar.ChangeNeiliAllocation(context, 0, -1);
			}
	...
	{
		if (dataKey.CharId != base.CharacterId || !base.CanAffect || dataKey.CustomParam0 != 0)
		{
			return 0;
		}
