...
namespace RealTimeModifyCharacterBackend
{
    public unsafe static class Util
    {
        public static GameData.Domains.Character.Character getCharacter(string charNameOrId)
        {
            ...
            {
                foreach (var character in ((Dictionary<int, GameData.Domains.Character.Character>)AccessTools.Field(typeof(CharacterDomain), "_objects").GetValue(DomainManager.Character)).Values)
                {
                    try
                    {
                        ...
                        if (charNameOrId.Equals(GetName_UIShow(ref nameRelatedData, character.GetId() == DomainManager.Taiwu.GetTaiwuCharId(), true)) ||
                            charNameOrId.Equals(GetName_UIShow(ref nameRelatedData, character.GetId() == DomainManager.Taiwu.GetTaiwuCharId(), false)))
                        {
                            id = character.GetId();
                            break;
                        }
        ...
        public static void SetGender(GameData.Domains.Character.Character character, sbyte gender, DataContext context)
        {
            ...
            AccessTools.Method(typeof(GameData.Domains.Character.Character), "SetModifiedAndInvalidateInfluencedCache").Invoke(character, new object[] { (ushort)3, context });
        ...
        public static void SetTransGender(GameData.Domains.Character.Character character, bool transgender, DataContext context)
        {
            ...
            AccessTools.Method(typeof(GameData.Domains.Character.Character), "SetModifiedAndInvalidateInfluencedCache").Invoke(character, new object[] { (ushort)13, context });
            if (character.CollectionHelperData.IsArchive)
            {
        ...
        {
            var name = internalGetName_UIShow(ref data, isTaiwu, getRealName);
        ...
        private static ValueTuple<string, string> internalGetName_UIShow(ref NameRelatedData data, bool isTaiwu, bool getRealName)
        {
            ...
            var name = data.FullName.GetName(data.Gender, DomainManager.World.GetCustomTexts());
        ...
        private static string internalGetMonasticTitle(ref NameRelatedData data, bool isTaiwu)
        {
            ...
            {
                var titles = LocalMonasticTitles.Instance.MonasticTitles;
            ...
            var name = internalGetName_UIShow(ref data, isTaiwu, false);
